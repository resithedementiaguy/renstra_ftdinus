    ...
    <div class="pagetitle">
        <nav>
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active">Kesesuaian Penelitian Program Studi</li>
            </ol>
        </nav>
    </div>
            ...
            <div class="col">
                <div class="card">
                    <div class="card-header text-white bg-success">
                        <h5 class="pt-2"><strong>Daftar Kesesuaian Penelitian Program Studi</strong></h5>
                    </div>
                    <div class="card-body">
                        <div class="alert alert-success alert-dismissible fade show" role="alert">
                            Silahkan untuk mengecek Kesesuaian Penelitian Program Studi Fakultas Teknik UDINUS Semarang
                        </div>
                        <div class="d-flex flex-column">
                            <!-- Select Option -->
                            <div style="width: 400px;" class="mb-3">
                                <label for="tahun" class="col-form-label">Tahun Data Kesesuaian Penelitian per Program Studi</label>
                        ...
                        <div class="tab-content" id="myTabContent">
                            <div class="tab-pane fade show active" id="elektro" role="tabpanel" aria-labelledby="elektro-tab">
                                <div class="row mt-4">
                                    <div class="col-6">
                                        <h5><strong>Total Hibah Penelitian Teknik Elektro</strong></h5>
                                            ...
                                            <?php foreach ($data_elektro as $elektro): ?>
                                                <tr>
                                                    ...
                                                    <td class="align-middle">
                                                        ...
                                                        <?php if (!empty($elektro->anggota_penelitian)): ?>
                                                            <?php foreach ($elektro->anggota_penelitian as $anggota): ?>
                            ...
                            <div class="tab-pane fade" id="industri" role="tabpanel" aria-labelledby="industri-tab">
                                <div class="row mt-4">
                                    <div class="col-6">
                                        <h5><strong>Total Hibah Penelitian Teknik Industri</strong></h5>
                                        <table class="table table-bordered">
                                            <tbody>
                                                <tr>
                                                    <td>Total Hibah Mandiri</td>
                                        ...
                                        <tbody>
                                            <?php foreach ($data_industri as $industri): ?>
                                                <tr>
                                                    ...
                                                    <td class="align-middle">
                                                        <!-- Iterasi anggota penelitian -->
                                                        <?php if (!empty($industri->anggota_penelitian)): ?>
                                                            <?php foreach ($industri->anggota_penelitian as $anggota): ?>
                                                                <?= htmlspecialchars($anggota->nama) ?>;
                                                            <?php endforeach; ?>
                                                        <?php endif; ?>
                                                    </td>
                            ...
                            <div class="tab-pane fade" id="biomedis" role="tabpanel" aria-labelledby="biomedis-tab">
                                <div class="row mt-4">
                                    <div class="col-6">
                                        <h5><strong>Total Hibah Penelitian Teknik Biomedis</strong></h5>
                                        <table class="table table-bordered">
                                            <tbody>
                                                <tr>
                                                    <td>Total Hibah Mandiri</td>
                                        ...
                                        <tbody>
                                            <?php foreach ($data_biomedis as $biomedis): ?>
                                                <tr>
                                                    ...
                                                    <td class="align-middle">
                                                        <!-- Iterasi anggota penelitian -->
                                                        <?php if (!empty($biomedis->anggota_penelitian)): ?>
                                                            <?php foreach ($biomedis->anggota_penelitian as $anggota): ?>
                                                                <?= htmlspecialchars($anggota->nama) ?>;
                                                            <?php endforeach; ?>
                                                        <?php endif; ?>
                                                    </td>
...
<script>
    function updateKesesuaian(id) {
        ...
        $.ajax({
            url: '<?= base_url("hasil_pelaporan/update_kesesuaian_penelitian/") ?>' + id,
            method: 'POST',
            data: {
    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        function fetchPenelitianData(year) {
            return $.ajax({
                url: '<?= base_url("hasil_pelaporan/get_kesesuaian_penelitian_data") ?>', 
                method: 'POST',
                data: { tahun: year },
                dataType: 'json'
            });
        ...
        function updateProgramTable(program, data) {
            ...
            data.forEach(function(item) {
                let anggotaNames = '';
                if (item.anggota_penelitian && item.anggota_penelitian.length > 0) {
                    anggotaNames = item.anggota_penelitian
                        .map(anggota => anggota.nama)
                        .join('; ');
                }

                const selectHtml = `
                    <select class="form-select" name="kesesuaian_penelitian" 
                            id="kesesuaian_penelitian_${item.id}" 
        ...
        if (currentYear) {
            updatePenelitianData(currentYear);
        }
